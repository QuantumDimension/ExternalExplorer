...
Q = pd.read_csv("src\\FileList.csv", sep=",")
#####################################################
...
with pd.ExcelWriter("dst\\FileList_Temp.xlsx", mode="w") as writer:
    Q.to_excel(writer, sheet_name = "Sheet1", index = False)
#####################################################################

### Open the new xlsx formatted document and select sheet ###
NewExcel = load_workbook(filename="dst\\FileList_Temp.xlsx")
...
NewExcel.save("dst\\Interactive_Excel_Prototype.xlsx")
##########################################################

### Clean up temp file ##############
os.remove("dst\\FileList_Temp.xlsx")
#####################################

### Clean up - Delete and Re-Create the source folder "src" ###
shutil.rmtree("src")
os.mkdir("src")
###############################################################
